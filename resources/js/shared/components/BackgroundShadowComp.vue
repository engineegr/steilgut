<template>
	<div
		class="background_shadow is_full_screen_width"
		:class="[getShadowModeStyleClass, zIndexClass]"
	></div>
</template>
...
export default {
	mixins: [validationErrors, mediaHelper, routeObserver],
	...
	methods: {
		$_checkedOrRedirected: function () {
			...
			if (!this.$_isInitialStateValid()) {
				console.warn(
					"Warn [TemplateComp::$_checkedOrRedirected] Returned Invalid Arguments"
				);
				this.$router.push({
					name: "controlPanel",
				});
	...
	computed: {
		getShadowModeStyleClass: function () {
			if (this.ifLightBackgroundPage()) {
				return "is_for_light_background";
			}
			return "is_for_dark_background";
		},
	},
};
</script>
